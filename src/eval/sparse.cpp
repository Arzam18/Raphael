...
void SparseIterator::add_nonzeros(VecU8 l0_out0, VecU8 l0_out1) {
    ...
    const auto mask =
        _mm512_kunpackw(
            nonzero_mask(l0_out1),
            nonzero_mask(l0_out0)
        );

    const auto idxs =
        _mm512_maskz_compress_epi16(mask, offset_);
    ...
    static_assert(USE_SIMD == 128);
    static_assert(ALIGNMENT == 16);
    static_assert(regw32 == 4);

    const u32 full_mask =
        (nonzero_mask(l0_out1) << regw32)
        | nonzero_mask(l0_out0);

    const u8 mask = static_cast<u8>(full_mask & 0xFF);

    const auto idxs =
        add_i16(
            offset_,
            load_i16(
                reinterpret_cast<const i16*>(&nonzero_idx[mask])
            )
        );
    ...
    for (i32 i = 0; i < regw32 / 4; i++) {
        ...
        _mm_storeu_si128(
            reinterpret_cast<__m128i*>(&indices_[count_]),
            idxs
        );
...
i32 SparseIterator::count() const {
    return count_;
}
